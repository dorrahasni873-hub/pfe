...
1. vehicules (vehicles)
   ...
   - marqueVoiture (varchar)
   - dateCirculation (date)
   - dateVisite (date)
   - dateTaxe (date)
   - etat (varchar): vehicle state
...
5. panne (breakdowns)
   - id_panne (PK, uuid)
   - typePanne (varchar): breakdown type
   - dateDeclaration (date)
   - chauffeurId (FK -> chauffeur)
   - matricule (FK -> vehicules)
   - status (varchar): en_attente/en_cours/resolue

6. affectation (assignments)
   - id_affectation (PK, uuid)
   - dateAffectation (date)
   - dateDebut (date)
   - typeAffectation (varchar)
   - etat (varchar)
...
Code:
\`\`\`js
const result = await db.select({ matricule: schema.vehicules.matricule, marqueVoiture: schema.vehicules.marqueVoiture, kilometrage: schema.maintenance.kilometrage }).from(schema.maintenance).innerJoin(schema.vehicules, eq(schema.maintenance.matricule, schema.vehicules.matricule)).orderBy(desc(schema.maintenance.kilometrage)).limit(1);
\`\`\`
...
export async function executeQuery(query: string) {
  ...
  const safeCode = code.replace(/\.all\(\)|\.execute\(\)/g, "");

  try {
    const asyncFunc = new AsyncFunction("db", "schema", "eq", "desc", "like", "count", "gte", "lte", "and", "or", "sql", safeCode);
    let results;
    try {
